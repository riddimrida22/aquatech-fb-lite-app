...
| ID | 🔒 | Decision | Rationale | Settled |
|---|---|---|---|---|
| D-033 | 🟡 | **iCloud contact sync is CardDAV, app-specific-password auth, credentials never persisted.** Apple has no contacts REST API, so sync uses **CardDAV** (`app/icloud_contacts.py`) against iCloud, authenticated with an **Apple ID + app-specific password** read from settings/env (`APPLE_ID`, `APPLE_APP_PASSWORD`) — never committed or stored in the DB. Contacts are linked to their iCloud vCard by `contacts.icloud_uid` (+ `icloud_etag`), with an `X-AQTPM-ID` vCard property as a fallback match. **Pull** imports iCloud → app (remote wins on non-blank fields; matched by uid → X-AQTPM-ID → email) then runs the D-032 dedup. **Push** writes each app contact up as a vCard 3.0 (create if no uid, else If-Match update). Endpoints (`GET /contacts/icloud/status`, `POST /contacts/icloud/pull`, `POST /contacts/icloud/push`) require `MANAGE_PROJECTS` and 400 when unconfigured. The live CardDAV HTTP client is isolated + unit-tested via a fake; it needs a real-credential smoke test on prod (the build env blocks egress to icloud.com). | Owner wants the consolidated BD address book reflected in iCloud; CardDAV + app-specific password is the only supported, safe path. | 2026-07-11 (proposed) |
