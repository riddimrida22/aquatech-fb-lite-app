        ...
        <table className="aq-lite-table">
          ...
          <tbody>
            {years.map((y) => {
              ...
              return (
                <tr key={y}>
                  <td><strong><SourceLink title={`Payroll pay periods · ${y}`} query={{ kind: "payroll", scope: "journal", start: `${y}-01-01`, end: `${y}-12-31` }}>{y}</SourceLink></strong></td>
                  ...
                  <td style={{ textAlign: "right" }}><SourceLink title={`Payroll gross · ${y}`} query={{ kind: "payroll", scope: "journal", start: `${y}-01-01`, end: `${y}-12-31` }}>{formatCurrency(v.totals.gross)}</SourceLink></td>
                  ...
                  <td style={{ textAlign: "right", fontWeight: 700, color: "var(--aq-green)" }}><SourceLink title={`Payroll employer cost · ${y}`} query={{ kind: "payroll", scope: "journal", start: `${y}-01-01`, end: `${y}-12-31` }}>{formatCurrency(v.totals.employer_cost)}</SourceLink></td>
                </tr>
              );
            })}
          </tbody>
              ...
              return (
                <>
                  ...
                  >
                    ...
                    <td style={{ textAlign: "right" }}><SourceLink title={`${emp} · payroll gross`} query={{ kind: "payroll", scope: "journal", q: emp }}>{formatCurrency(vals.gross)}</SourceLink></td>
                    ...
                    <td style={{ textAlign: "right", fontWeight: 700, color: "var(--aq-green)" }}><SourceLink title={`${emp} · employer cost`} query={{ kind: "payroll", scope: "journal", q: emp }}>{formatCurrency(vals.employer_cost)}</SourceLink></td>
                          ...
                          <tbody>
                            {Object.entries(vals.by_year).sort(([a],[b])=>a.localeCompare(b)).map(([y, t]) => (
                              <tr key={y}>
                                ...
                                <td style={{ textAlign: "right" }}><SourceLink title={`${emp} · payroll gross · ${y}`} query={{ kind: "payroll", scope: "journal", q: emp, start: `${y}-01-01`, end: `${y}-12-31` }}>{formatCurrency(t.gross)}</SourceLink></td>
                                <td style={{ textAlign: "right" }}>{formatCurrency(t.employer_taxes)}</td>
                                <td style={{ textAlign: "right" }}>{formatCurrency(t.employer_401k)}</td>
                                <td style={{ textAlign: "right", fontWeight: 600 }}><SourceLink title={`${emp} · employer cost · ${y}`} query={{ kind: "payroll", scope: "journal", q: emp, start: `${y}-01-01`, end: `${y}-12-31` }}>{formatCurrency(t.employer_cost)}</SourceLink></td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
